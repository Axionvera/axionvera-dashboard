...
function DashboardContent({ isConnected, publicKey }: { isConnected: boolean; publicKey?: string | null }) {
  const {
    balance,
    rewards,
    transactions,
  ...
  const [liveState, setLiveState] = useState<LiveVaultState | null>(null);
  const [liveError, setLiveError] = useState<string | null>(null);

  const liveUser =
    publicKey || process.env.NEXT_PUBLIC_AXIONVERA_TEST_WALLET_ADDRESS || "";
  ...
  useEffect(() => {
    if (!hasMounted || !AXIONVERA_VAULT_CONTRACT_ID) return;
    ...
    async function loadLiveState() {
      try {
        setLiveError(null);
        const query = liveUser ? `?user=${encodeURIComponent(liveUser)}` : "";
        const response = await fetch(`/api/vault/live-state${query}`, {
          signal: controller.signal,
        });

        if (!response.ok) {
          const payload = await response.json().catch(() => ({} as { error?: string }));
          throw new Error(payload.error || "Failed to load live vault state");
        }

        const payload = (await response.json()) as LiveVaultState;
        if (!cancelled) setLiveState(payload);
      } catch (err) {
        if (!cancelled && !(err instanceof DOMException && err.name === "AbortError")) {
          setLiveError(err instanceof Error ? err.message : "Failed to load live vault state");
        }
      }
    }
  ...
  return (
    <div className="mt-8 grid gap-6">
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="Vault Balance"
          value={isLoading ? "Loading…" : displayVaultBalance}
          helper="Current vault balance from the configured vault adapter."
          tone="success"
        />
        <MetricCard
          label="Claimable Rewards"
          value={isLoading ? "Loading…" : displayRewards}
          helper="Rewards available through the configured vault adapter."
          tone="warning"
        />
        <MetricCard
          label="Network"
